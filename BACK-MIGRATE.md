...
- **Scheduler** (train.py:869): top-level `lr_scheduler` ∈ {`constant` (default), `linear`, `cosine`, `wsd`} — **cosine IS available** (plan said only constant/linear). Plus `warmup_steps` (int) and `force_constant_lr` (float, overrides everything even on resume). No rex/restarts/custom schedulers. `warmup_ratio` → compute `warmup_steps` from dataset size (reuse existing logic). `wsd` (warmup-stable-decay) adds `wsd_warmup_fraction` (0.01), `wsd_decay_fraction` (0.10) and `wsd_eta_min` (0.0), all fractions of total steps; it brings its own warmup, so `warmup_steps` is ignored under `wsd`.
...
**Converter (`tools/convert_sdscripts_toml.py`):** pure-dict `convert(loaded) -> (new, warnings)` + `is_old_format(loaded)` detector — `network_args.args.type` present ⇒ definitively new format (early False), since new-UI saves used to carry stray legacy keys (`qwen3_max_token_length`, `[[datasets]]` multi-res persistence) that misdetected them as old and let the converter clobber the adapter config to LoRA/rank-32; GeneralUI also no longer emits `t5_tokenizer_path`/`qwen3_max_token_length`/`t5_max_token_length` (controls hidden). Plus a CLI (`python tools/convert_sdscripts_toml.py old.toml [new.toml]`). Wired into `MainUI.process_toml` as an auto-fallback: old-format configs are detected, converted, and the dropped/remapped-args report is both printed and shown in a warning dialog on the UI load path. Verified with a pure-dict test (timestep/adapter/optimizer/bucket/sample/logging/subset mappings + all drops) **and** a full round-trip (old TOML → convert → load into new widgets → payload → backend `validate()`).
